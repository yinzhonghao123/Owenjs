...
var tableHeader=[
    {header:" 终端名称 ",dataIndex:"terminalName",width:"30%"},                       //header表示表头显示文字，dataIndex对应该行显示数据的key
    {header:"经度",dataIndex:"longitude",width:"300px"},                               //width定义列宽，可以是%也可以是px
...
var vm = new Vue({
    ...
    methods:{
        getdata:function(start,size,orderIndex,orderType){ //表格数据获取方法，此处可通过Ajax或vue-resource进行数据请求
            console.log(orderIndex+","+orderType)
            return {
                "tableData":tableData,
                "recordTotal":100,
                "startPage":start,
